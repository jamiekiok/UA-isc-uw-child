...
(function($) {
	...
	$.fn.filterTable = function(options) { // define the filterTable plugin
		...
		function doFiltering (table, q, type) { // handle the actual table filtering
				...
				if (q==='') { // if the filtering query is blank
				...
				} else if (q !== "choose unit") { // if the filter query is not blank
					...
					if (type === "unit") {
						tbody.find('td.ua-staff-directory-unit').removeClass(settings.highlightClass).filter(':filterTableFind("'+q.replace(/(['"])/g,'\\$1')+'")').addClass(settings.highlightClass).closest('tr').show().addClass(settings.visibleClass); // highlight (class=alt) only the cells that match the query and show their rows
					}else {
						$("#ua-staff-directory-unit-select").prop("selectedIndex", 0);
		...
		return this.each(function() {
			...
			if (t[0].nodeName==='TABLE' && tbody.length>0 && (settings.minRows===0 || (settings.minRows>0 && tbody.find('tr').length>settings.minRows)) && !t.prev().hasClass(settings.containerClass)) { // only if object is a table and there's a tbody and at least minRows trs and hasn't already had a filter added
				...
				$("#ua-staff-directory-unit-select").bind("change", function() {
						filter.val("");
					if($(this).val() === "choose unit") {
						filter.trigger("click");//do a blank search
					} else {
						doFiltering(t, $(this).val(), "unit");	
					}
				});
